...
class MongoDB {
    ...
    async init() {
        try {
            this.client = new mongo.MongoClient(this.databaseURL, {useNewUrlParser:true, useUnifiedTopology:true});
            await this.client.connect();
            this.db = this.client.db(this.databaseName); 
            let multimediaItemsCol = await this.collection("multimedia_items");
            await multimediaItemsCol.createIndex({capa:1, tiempo:1});
        } catch(error) {
