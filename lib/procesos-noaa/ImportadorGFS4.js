...
import { GFS4ModelExecution } from "./GFS4ModelExecution.js";
import fs from "fs";
...
class ImportadorGFS4 extends ZRepoProcess {
    constructor() {
        super();
        this.declareParams([]);
    }

    get code() {return "noaa-gfs4"}
    get name() {return "Importador NOAA - GFS4"}
    get description() {return "Importador NOAA - GFS4"}

    get nParallelDownloads() {return parseInt(process.env.GFS4_N_PARALLEL_DOWNLOADS || "5")}
    get nRetries() {return parseInt(process.env.N_RETRIES || "5")}
    get maxForecastHours() {return parseInt(process.env.GFS4_MAX_FORECAST_HOURS || "240" || "384")}


    async exec(params) {
        try {
            await this.addLog("I", "Iniciando proceso NOAA - GFS4");
            ...
            if (this.state.files) {
                // Chequear si están con la url anterior y transformar a la nueva
                let oldUrl = "https://www.ftp.ncep.noaa.gov/data/nccf/com/gfs/prod";
                let newUrl = "https://noaa-gfs-bdp-pds.s3.amazonaws.com";
                for (let f of this.state.files) {
                    if (f.url.startsWith(oldUrl)) {
                        let url = newUrl + f.url.substring(oldUrl.length);
                        console.log("cambia url: ", f.url, "=>", url);
                        f.url = url;
                    }
                }
            ...
            if (this.state.currentModel) {
                ...
                mExec = new GFS4ModelExecution(modelTime);
            ...
            if (!mExec) {
                mExec = new GFS4ModelExecution(moment.tz("UTC").startOf("hour"));
                let n = 0,published = false;
                do {
                    console.log("buscando ...");
                    published = await mExec.isPublished(this);
                    console.log("buscó: " + published);
    ...
    downloadFile(file) {        
        return new Promise(async (resolve, reject) => {
            try {
                ...
                await this.addLog("I", fileName + ": Iniciando Descarga");
