...
class ZRepoPluginClient {
    ...
    async init(app, zRepoURL, pluginCode, pluginPwd, callbackURL) {
        this.zRepoURL = zRepoURL;
        this.pluginCode = pluginCode;
        this.pluginPwd = pluginPwd;
    ...
    async registerEndPoints(app) {
        try {
            app.post("/zcb/" + this.pluginCode + "/ping", (req, res) => this.resolvePing(req, res));
    ...
    async registerPlugin() {
        try {
            console.log("Registrando plugin " + this.pluginCode + " en " + this.zRepoURL + " ...");
            this.sessionToken = await this.post(this.zRepoURL + "/pluginAPI/register", null, {
                code:this.pluginCode, pwd:this.pluginPwd, callbackURL: this.callbackURL, processes:this.getProcMetadata()
            });
            this.token = this.sessionToken.token;
...
class ZRepoProcess {
    ...
    async checkCanceled() {
        try {
            ...
            if (s.canceling) throw "Finaliza por Cancelicación solicitada por usuario";
        } catch (error) {
            throw error;
        }
    }
