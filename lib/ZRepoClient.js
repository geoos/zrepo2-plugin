...
class ZRepoClient {
    ...
    async accumDataSet(dsCode, row) {
        try {
            if (!this.batchAccum) this.batchAccum = {};
            let rows = this.batchAccum[dsCode];
            if (!rows) {
                rows = [];
                this.batchAccum[dsCode] = rows;
            }
            rows.push(row);
            if (rows.length >= MAX_BATCH_ROWS) {
                await this.flushDataSet(dsCode);
            }
        } catch(error) {
            throw error;
        }
    }
    async flushDataSet(dsCode) {
        try {
            if (!this.batchAccum) this.batchAccum = {};
            let rows = this.batchAccum[dsCode];
            if (!rows || !rows.length) return;
            await this.postDataSet(dsCode, rows);
            delete this.batchAccum[dsCode];
        } catch (error) {
            throw error;
        }
    }
    async deleteDataSetRows(dsCode, startTime, endTime, filter) {
        try {
            let url = this.url + "/dataSet/" + dsCode + "?startTime=" + startTime + "&endTime=" + endTime + "&filter=" + encodeURIComponent(JSON.stringify(filter || {}));
            let res = await this.post(url, null, {}, true);
            return JSON.parse(res).n;
        } catch (error) {
            throw error;
        }
    }
    ...
    // Variables
    deletePeriod(varCode, startTime, endTime, varData, details, filter) {
